...
[标准](http://eel.is/c++draft/cpp.include)并未指定编译器是怎样从 `#include` 指令中的标识符唯一定位头文件的，它也没有指定由什么构成唯一性。例如，实现是否将标识符视为大小写敏感，或标识符是否是到头文件的文件系统路径，以及若是如此，文件系统路径的层次是如何分隔的。
